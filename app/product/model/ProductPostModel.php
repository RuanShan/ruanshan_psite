...
class ProductPostModel extends Model
{
     ...
     */
    public function adminAddItem($data, $categories)
    {
     ...
     * @throws \think\Exception
     */
    public function adminEditItem($data, $categories)
    {
    ...
    public function addTags($keywords, $itemId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('product_tag_post')->where('post_id', $itemId)->column('tag_id');

            foreach ($keywords as $keyword) {
                $keyword = trim($keyword);
                if (!empty($keyword)) {
                    $findTag = $productTagModel->where('name', $keyword)->find();
                    if (empty($findTag)) {
                        $tagId = $productTagModel->insertGetId([
                            'name' => $keyword
                        ]);
                    } else {
                        $tagId = $findTag['id'];
                    ...
                    if (!in_array($tagId, $oldTagIds)) {
                        array_push($data, ['tag_id' => $tagId, 'post_id' => $itemId]);
                    }
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('product_tag_post')->where('post_id', $itemId)->delete();
            }
