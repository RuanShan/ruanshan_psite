...
class Product extends TagLib
{
    ...
    protected $tags = [
        ...
        'tagitems'      => ['attr' => 'field,where,limit,order,page,relation,returnVarName,pageVarName,tagId', 'close' => 1],//非必须属性item
    ...
    public function tagItems($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::items([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
     ...
     * 标签产品列表标签
     */
    public function tagTagItems($tag, $content)
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $order         = empty($tag['order']) ? 'post.published_time DESC' : $tag['order'];
        $relation      = empty($tag['relation']) ? '' : $tag['relation'];
        $pageVarName   = empty($tag['pageVarName']) ? '__PAGE_VAR_NAME__' : $tag['pageVarName'];
        $returnVarName = empty($tag['returnVarName']) ? 'tag_items_data' : $tag['returnVarName'];
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::tagItems([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagPage($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'product_page' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::page({$id});
?>
{$content}
parse;
        return $parse;
...
parse;
    ...
    public function tagCategory($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'product_category' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::category({$id});
?>
{$content}
parse;
        return $parse;
