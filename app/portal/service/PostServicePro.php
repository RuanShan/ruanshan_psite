...
class PostServicePro
{
     ...
     */
    public function adminArticleList($filter, $postType)
    {
        return $this->adminPostList($filter, $postType);
    }
     ...
     * @throws \think\exception\DbException
     */
    public function adminPostList($filter, $postType = 1)
    {
        ...
        $articles        = $portalPostModel->alias('a')->field($field)
            ...
            ->where(function (Query $query) use ($filter, $postType) {
                ...
                if ($postType) {
                    $query->where('a.post_type', $postType);
                } else {
