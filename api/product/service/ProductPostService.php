...
class ProductPostService
{
     ...
     */
    public function postItems($filter, $isPage = false)
    {
        $join = [];
        ...
        if (!empty($category)) {
            array_push($join, ['__PRODUCT_CATEGORY_POST__ b', 'a.id = b.post_id']);
        ...
        if (!empty($page)) {
            $productPostModel = $productPostModel->page($page);
        } elseif (!empty($limit)) {
            $productPostModel = $productPostModel->limit($limit);
        } else {
            $productPostModel = $productPostModel->limit(10);
        }

        $items = $productPostModel
            ->alias('a')
            ->field($field)
            ->join($join)
