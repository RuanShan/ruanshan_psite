...
class PluginModulesModel extends Model
{
    ...
    /**
     * 获取某个内容模型下所有表
     * @param $modules_id 默认当前模型
     */
    public function getModuleTables($modules_id = 0){
        $modules_id = $modules_id ? $modules_id : $this->id;
        $rs = cache('PluginModulesModel.getModuleTables'. $modules_id);
    ...
    public function getTables($cache = false){
        ...
        //查询配置表
        $tables = $this->name('plugin_modules_tables')->where('modules_id', $this->id)->field('table_name,relate_field,relate_level,remark,control_edit')->select();
        $table_tmp = [];
        foreach($tables as $v){
        ...
        foreach($tableList as $v){
            ...
            if(isset($table_tmp[$v['Name']])){
                ...
                $v['control_edit'] = !is_array($table_tmp[$v['Name']]['control_edit']) ? json_decode($table_tmp[$v['Name']]['control_edit'], true) : [];
            }else{
                $v['table_name']   = $v['Name'];
                $v['relate_field'] = '';
                $v['relate_level'] = 0;
        ...
        array_multisort(array_column($rs, 'relate_level'), SORT_ASC, $rs);
