...
class EditForm{
    ...
    public static function start($tag, $table=''){
        ...
        $tag['config']    = isset($tag['config']) ? $tag['config'] : '';//表单配置设置
        ...
        switch($type){
            ...
            case 'ctime':
            case 'cdate':
                if(empty($tag['value']) && in_array($tag['config'], ['ctime','system_time'])){
                    $tag['value'] = time();//默认配置时间
                }
            default:
    ...
    public function text($tag){
        if ( empty($tag['config']) ) {
            echo <<<parse
            <input type="text" class="form-control" name="{$tag['postname']}" value="{$tag['value']}" style="width:{$tag['width']}">
parse;
        } elseif( in_array(mb_substr($tag['config'],0,1), ['(','（']) ) { // "placeholder" 或者 "(单位)"
            $tag['config'] = str_replace(['(',')','（','）'], '', $tag['config']);
            echo <<<parse
            <div class="input-group" style="width:{$tag['width']}">
                <input type="text" class="form-control" name="{$tag['postname']}" value="{$tag['value']}" placeholder="{$tag['config']}">
                <span class="input-group-addon" style="max-width:{$tag['width']};overflow:hidden;">{$tag['config']}</span>
            </div>
parse;
        } else {
            echo <<<parse
            <input type="text" class="form-control" name="{$tag['postname']}" value="{$tag['value']}" placeholder="{$tag['config']}" style="width:{$tag['width']}">
parse;
        }
    }
...
parse;
    ...
    public  function cdate($tag){
        if($tag['value']>19700100 && $tag['value']<21990100){
            $tag['value'] = substr($tag['value'],0,4).'-'.substr($tag['value'],4,2).'-'.substr($tag['value'],6);
        }else{
            $tag['value'] = ctype_alnum($tag['value'])? date('Y-m-d', $tag['value']) : $tag['value'];
        }
        echo <<<parse
...
parse;
    ...
    public  function system($tag){
        $type = !empty($tag['config']) ? $tag['config'] : '';
        echo <<<parse
...
parse;
    ...
    public  function category($tag){
        ...
        $pid = empty($tag['config']) ? 0 : intval($tag['config']);
        ...
        echo <<<parse
            ...
            <input class="form-control" type="hidden"  value="{$tag['value']}" name="{$tag['postname']}"  id="js-categories-id-{$tag['name']}"/>
parse;
        ...
        if(!isset($this->doSelectCategory)){
            ...
            <script>//分类
                function doSelectCategory(fieldname, pid) {
                    ...
                    openIframeLayer("{$url}?fieldname="+fieldname+"&pid="+pid+"&ids=" + selectedCategoriesId, '请选择', {
                        area: ['700px', '400px'],
                        btn: ['确定', '取消'],
...
parse;
    ...
    public function editor($tag){
        echo <<<parse
            ...
            <script type="text/plain" id="content" name="{$tag['postname']}[value]" style="width:{$tag['width']};min-height:400px;">{$tag['value']}</script>{$tag['desc']}
parse;
        if(!isset(self::$instance->editor_exist)){
            ...
            echo <<<parse
                    ...
                    $(function () {
                        ...
                        //editorcontent.sync();
                        //去掉图片除了src之外的所有属性
                        editorcontent.addListener('afterpaste', function () {
                            setTimeout(
                                function(){
                                    var html = editorcontent.getContent();
                                    html = html.replace(/<img.+?src="(.+?)".+?>/gi,'<img src="$1">');
                                    editorcontent.setContent(html);
                                }
                            ,500);
                        });
                        //修改后才提交检测
                        ...
                        editorcontent.addListener("contentChange",function(){
                                $("#content").attr("name","{$tag['postname']}");
                        });
                    });
                    </script>
parse;
    ...
    public static function uploadVideo($tag){
        ...
        if(!empty($video_arr)){//编辑
            ...
            <div id="saved-video-{$name}-{$key}" style="margin-bottom:2px">
                ...
                <video id="video-{$key}-preview" src="$video_url" style="max-width: 320px;cursor: pointer" controls="controls"></video>
...
parse;
        ...
        echo <<<parse
            ...
            <div id="saved-video-{$name}-{id}" style="margin-bottom:2px">
                ...
                <video id="video-{id}-preview" src="{url}" style="max-width: 320px;cursor: pointer" controls="controls"></video>
